...
ADD target/*.jar $WORK_PATH/app.jar

ENV MEM_OPTS="-Dspring.profiles.active=uat"
ENV APOLLO_ENV=""
...
ENTRYPOINT java ${JAVA_OPTS} ${MEM_OPTS} -Denv=${APOLLO_ENV} -Dapollo.configService=${APOLLO_CONFIG_SERVICE} -jar -Djava.security.egd=file:/dev/./urandom app.jar